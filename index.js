...
bot.command("user", (res) => {
    ...
    if (res.message) {
      ...
      res.reply(`Hello ${res.message.from.first_name}! ID: ${res.message.from.id}.You can try for /add task in trello board or /remove task from trello board , you want to see your all task /getmytask or you can visit Trell board /open_my_trello_board `);
    } else {
...
bot.use(async (res) => {
    if (res.message.text.includes("addTask")) {
      ...
      res.reply(await RemoveTaskFromTelegram(user_id, TaskId[1]));
        ...
        const user_id = res.message.from.id;
        const threadId = res.message.thread_id;
        console.log(TaskId[1],user_id);
         // Create the board
 trello.post('/1/boards/', { name: TaskId[1] }, (err, data) => {
    // if (err) {
    //   res.sendMessage(user_id, `Error creating board: ${err} "text":"any text"`);
    // } else {
    //     let test="any world"
    //   res.sendMessage(user_id,test,{ thread_id: "56225866", parse_mode: 'HTML' });
    // }
    Boardid=data.id
    trelloUrl=data.url
  // console.log(data)
         ...
         res.reply(`board created to create a list on board:- ${TaskId[1]} /create_list `);
 } 
...
trello.post(`/1/lists`, { name: TaskId[2], idBoard:Boardid }, (err, data) => {
  // if (err) {
  //   // bot.sendMessage(chatId, `Error adding card: ${err}`);
  //   console.log(err)
  // }
  // } else {
 ...
 // console.log(data)
     ...
     res.reply(`list created to create a card on list:- ${TaskId[2]} /create_card `);
} 
  ...
  trello.get(`/1/boards/${Boardid}/lists`, (err, lists) => {
    ...
    if (!list) {
      // bot.sendMessage(chatId, `List not found: ${listName}`);
      // return;
      res.reply(`list not found`)
    }
//console.log("we can createcard")
    ...
    trello.post(`/1/cards/`, { name: TaskId[2], idList: Listid }, (err, data) => {
     ...
     res.reply(`card created to check go on trello by click ${trelloUrl}  `);
} 
